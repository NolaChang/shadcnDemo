<template>
  ...
  const chagePage = async (page: number) => {
    current_page.value = page;
    // 當切換頁面時，重設當前頁面的選擇狀態
    selectedItems.value = Array(show_table.value.length).fill(false);
    await axios
      .get("/mock/getUserList", { params: { page: page } })
      .then((res: any) => {
        setTimeout(() => {
          // console.log(res.data);
          show_table.value = res.data.data.users;
        }, 0);
      })
      .catch((error) => {
        console.log(error);
  ...
  watch(
    ...
    () => {
      ...
      show_table.value = props.content;
      removeListener();
      addListener();
      addSortBtn();
    },
  ...
  const removeListener = () => {
    ...
    items.forEach((item, index) => {
      item.removeEventListener("contextmenu", () => {console.log('1')});
    });
  };
  const addListener = () => {
